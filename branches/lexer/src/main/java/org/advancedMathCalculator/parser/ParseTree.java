 ...
 * computations of values based off of indexes passed in. ie The ParseTree
 * represents a function f and you can insert a value x such that the result y =
 * f(x). X can be any value inside of the domain for the defined expression. If
 * X is outside of the domain the result is undefined.
 * 
 * @author Ryan
 * 
 */
public class ParseTree implements Calculate {
	...
	public static ParseTree generate(String expr, boolean optimize)
	...
	private final LinkedBlockingQueue<EquationToken> queue;

	/**
	 * Creates a ParseTree object that has a unique expression. Once constructed
	 * the tree is generated and optimized.
	 * 
	 * @param expression
	 *            The mathematical expression to be parsed into a binary tree.
	...
	/**
	 * Creates a ParseTree object that has a unique expression. Once constructed
	 * the tree is generated and or optimized.
	 * 
	 * @param expression
	 *            The mathematical expression to be parsed into a binary tree.
	 * @param optimize
	 *            A boolean stating weather the expression tree should be
	 *            optimized or not.
	 * @throws ParseTreeGenerationException
	 */
	...
	/**
	 * Removes implicit multiplication from the expression and makes it
	 * explicit. Edits expression.
	 */
	private void enqueue() {
		final EquationLexer lexer = new EquationLexer(this.expression);
		...
		while (lexer.hasMoreElements()) {
			...
			if (ExpressionType.isTerm(current.getType())) {
				...
				if (ExpressionType.isTerm(next.getType())
						|| ExpressionType.isTerm(next.getType())
						|| (next.getType() == ExpressionType.LEFTPAREN)) {
	...
	/**
	 * Computes a value by replacing every variable in the tree with the passed
	 * in index.
	 * 
	 * @param index
	 *            The index to compute a value on given the expression that the
	...
	private BinaryTree<EquationToken> fourthLevel() throws ParseException {
		...
		if ((next != null) && ExpressionType.isTerm(next.getType())) {
			...
			if (tempNext.getType() == ExpressionType.LEFTPAREN) {
				...
				if (tempNext.getType() != ExpressionType.RIGHTPAREN) {
					throw new ParseException("Expected "
							+ ExpressionType.RIGHTPAREN + " but got "
							+ tempNext.getToken(), tempNext.getPosition());
				}

				return mkNode(next, t);
			} else {
				throw new ParseException("Expected " + ExpressionType.LEFTPAREN
						+ " but got " + tempNext.getToken(),
						tempNext.getPosition());

			}

		} else if ((next != null)
				&& next.getType().equals(ExpressionType.LEFTPAREN)) {
			next = nextToken();
			t = rootLevel();
			next = nextToken();
			if (!next.getType().equals(ExpressionType.RIGHTPAREN)) {
				throw new ParseException("Expected "
						+ ExpressionType.RIGHTPAREN + " but got "
						+ lastToken.getToken(), lastToken.getPosition());
			}
			return t;
		} else if ((next != null)
			...
			throw new ParseException(
					"Unexpected token " + lastToken.getToken(),
	...
	/**
	 * Creates a subtree where the first left child is Double.Nan for computing
	 * the unary - value, and for functions.
	 * 
	 * @param next
	 * @param t
	 * @return
	...
	private BinaryTree<EquationToken> mkNode(EquationToken next,
			BinaryTree<EquationToken> t) {
		final BinaryTree<EquationToken> t1 = new BinaryTree<EquationToken>();
		t1.setRoot(new TreeNode<EquationToken>(new EquationToken("Nan",
				ExpressionType.NAN)));
		return mkNode(next, t1, t);
	}

	/**
	 * Creates a subtree where next is the root, t is the left subtree, and t1
	...
	private BinaryTree<EquationToken> mkNode(EquationToken next,
		...
		final BinaryTree<EquationToken> retTree = new BinaryTree<EquationToken>();
		final TreeNode<EquationToken> newNode = new TreeNode<EquationToken>(
				next);
	...
	private double optimize(TreeNode<EquationToken> current)
		...
		if ((current.getLeft() == null) && (current.getRight() == null)) {
			if (current.getItem().getType().equals(ExpressionType.VARIABLE)) {
				...
				return ExpressionType.eval(current.getItem(), 0, 0, 0);// This
																		// should
																		// only
																		// be
				// numeric values that
			...
			if (current.getLeft() != null) {
				...
				} catch (final OptimizeException e) {
					retException = e;
					isException = true;
				}
			}
			if (current.getRight() != null) {
				try {
					...
					if (isException) {
						final TreeNode<EquationToken> tempNode = new TreeNode<EquationToken>(
								new EquationToken("" + rightRet,
			...
			return ExpressionType.eval(current.getItem(), leftRet, rightRet, 0); // There
																					// should
																					// be
																					// no
																					// index.
	...
	/**
	 * Attempts to optimize the tree to reduce the time when computing a value
	 * in the tree.
	 */
	private void optimizeTree() {

		try {
			final double retVal = optimize(bTree.getRoot());
			final TreeNode<EquationToken> tempNode = new TreeNode<EquationToken>(
					new EquationToken("" + retVal, ExpressionType.NUMBER));
			bTree.setRoot(tempNode);
		} catch (final OptimizeException e) {
			// I should not need to do anything, This just means that not all of
			// the
	...
	private EquationToken peek() throws ParseException {
		final EquationToken next = queue.peek();
		if (next == null) {
			return null;
		}
		if (next.getType() == ExpressionType.BAD_TOKEN) {
	...
	private BinaryTree<EquationToken> rootLevel() throws ParseException {
		...
		EquationToken next = peek();
		while ((next != null)
				&& (next.getType().equals(ExpressionType.ADD) || next.getType()
						.equals(ExpressionType.SUBTRACT))) {
			next = nextToken();
			final BinaryTree<EquationToken> t1 = secondLevel();
			t = mkNode(next, t, t1);
	...
	private BinaryTree<EquationToken> secondLevel() throws ParseException {
		...
		EquationToken next = peek();
		while ((next != null)
				&& (next.getType().equals(ExpressionType.MULTIPLY) || next
						.getType().equals(ExpressionType.DIVIDE))) {
			next = nextToken();
			final BinaryTree<EquationToken> t1 = thirdLevel();
			t = mkNode(next, t, t1);
	...
	private BinaryTree<EquationToken> thirdLevel() throws ParseException {
		...
		if ((next != null) && next.getType().equals(ExpressionType.POW)) {
			...
			final BinaryTree<EquationToken> t1 = fourthLevel();
