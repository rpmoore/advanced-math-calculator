...
public class ExpressionLexer implements Lexer<EquationToken> {
	...
	public EquationToken nextElement() {
		final EquationToken curr = lexer.peek();
		if (next != null) {
			final EquationToken ret = next;
			next = null;
			prev = ret;
			return ret;
		}
		if ((prev != null)
				&& (curr.getType().isTerm() && prev.getType().isTerm())) {
			final EquationToken ret = new EquationToken("*",
					ExpressionType.MULTIPLY);
			prev = ret;
			return ret;
		}
		if ((curr.getType() == ExpressionType.SUBTRACT)
				&& ((prev != null) && prev.getType().isTerm())) {
			final EquationToken ret = new EquationToken("+", ExpressionType.ADD);
			prev = ret;
